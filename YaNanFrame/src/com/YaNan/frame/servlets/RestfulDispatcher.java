...
public class RestfulDispatcher extends HttpServlet implements ServletDispatcher,ServletContextListener{
     ...
     * 1、获取真实的servletBean（url需要进行重组，否则不会识别 get post delete put等请求）
     * 2、获取封装参数 （从servletBean.getParameters()方法获取需要的参数，遍历每个参数，通过其注解或属性从PlugsFactory
     * 		获取对应参数处理器，获取参数,并封装，拦截器拦截的是ParameterHandler的getParameter方法，通过对其传入第一个参数
     * 		参数获取到servlet类中对应方法的参数，再获取参数的输入 Constraint.class的注解，对该注解输出结果进行验证并作出对
     * 		应响应）
     * 3、调用方法（将第二步获取到的参数进行校验，没问题之后通过反射调用其方法）
     * 4、处理响应
     */
	@Override
				...
				Object handlerResult = invokeProxyMethhod(request, response, servletBean, proxyObject, parameters);
					...
					if(resultAnnotations==null||resultAnnotations.isEmpty()){
						if(handlerResult==null){
							...
							ResponseHandler responseHandler = PlugsFactory.getPlugsInstanceByAttributeStrict(ResponseHandler.class, handlerResultType.getName());
						...
						ResponseHandler responseHandler = PlugsFactory.getPlugsInstanceByAttributeStrict(ResponseHandler.class, annotationType.getName());
						if(responseHandler==null)
							throw new ServletException("could not found response handler for response type "+annotationType.getName());
     ...
     * @throws Exception 
     */
	protected List<Object> urlencodedParameterBind(HttpServletRequest request, HttpServletResponse response,ServletBean servletBean, Map<String, ?> model) throws Exception{
			...
			Iterator<Entry<Parameter, Map<Class<Annotation>, List<Annotation>>>> paramIterator = servletBean.getParameters().entrySet().iterator();
			...
			while(paramIterator.hasNext()){ 
				...
				if(paramEntry.getValue()!=null){
					...
					if(annos!=null&&annos.size()!=0){
						...
						parameters.add(parameterHandler==null?null:parameterHandler.getParameter(paramEntry.getKey(),parameterAnnotation));
						continue;
					}
				}
				ParameterHandler parameterHandler = parameterHandlerCache.getParameterHandler(paramEntry.getKey().getType());
	...
	public String getUrlMapping(HttpServletRequest request) throws ServletException{
		...
		if(method.equals("GET")){
			...
			if(method==null){
				...
				throw new ServletException("servlet handle not process this request method ! request url :"+urlMapping+",method:"+method);
			}
