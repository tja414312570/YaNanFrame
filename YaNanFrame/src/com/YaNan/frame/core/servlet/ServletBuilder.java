	...
	public void initByScanner(){
		...
		scanner.doScanner(new ClassInter(){
			...
			public void find(Class<?> cls) {
				RequestMapping parentRequestMaping = cls.getAnnotation(RequestMapping.class);
				Method[] methods = cls.getMethods();
				for(Method method :methods){
					try {
						Action action = method.getAnnotation(Action.class);
						if(action!=null){
								ServletBean servletBean = ServletBeanBuilder.builder(action,method,parentRequestMaping);
								servletMannager.add(servletBean);
								continue;
						}
						RequestMapping requestMapping = method.getAnnotation(RequestMapping.class);
						if(requestMapping!=null){
							ServletBean servletBean = ServletBeanBuilder.builder(requestMapping, method, parentRequestMaping);
							servletMannager.add(servletBean);
							continue;
						}
					} catch (Exception e) {
