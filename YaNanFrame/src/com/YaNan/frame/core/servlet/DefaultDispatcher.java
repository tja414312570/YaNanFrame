...
//								reObj = loader.invokeMethod("exception",new Class<?>[]{Exception.class},e);
							...
							response.getWriter().print("<title>ACTION ERROR</title><h1 style='color:#F00'>Action error at \""+servletName+"\" at namespace \""+namespace+"\"<h1><p style='font-size:16px'>");
			...
			response.setStatus(500);
			response.getWriter().print("<title>ACTION ERROR</title><h1 style='color:#F00'>Could not find action \"" + servletName+"\" at namespace \""+namespace+"\"<h1><p style='font-size:16px'>");
			log.error("could not find servlet :" + servletName+" at namespace "+namespace);
		}
