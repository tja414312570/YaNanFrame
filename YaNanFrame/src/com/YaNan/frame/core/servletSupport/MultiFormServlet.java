...
public class MultiFormServlet extends DefaultServlet {
	...
	public void MultiFormSupport(ClassLoader loader,ServletBean bean){
		try {
			if (ServletFileUpload.isMultipartContent(this.RequestContext)) {
				...
				if(uploadConf!=null&&uploadConf.MaxSize()!=-1&&this.RequestContext.getContentLength()>uploadConf.MaxSize()){
					//文件总大小超过限制
	...
	public void isUploadField(FileItem fileItem,ClassLoader loader,Upload uploadConf,File uploadPath) throws IOException, InvalidKeyException, SecretKeyNotExists, IllegalBlockSizeException, BadPaddingException{
		InputStream inputStream = fileItem.getInputStream();
	//		File uploadPath = new File(request.getServletContext().getRealPath("/")+uploadDir);
		File fileTmp = null;
		if(uploadPath.exists())uploadPath.mkdirs();
		if(uploadConf!=null){
			fileTmp = this.doUpload(fileItem, inputStream,loader, uploadConf);
		}else{
			 fileTmp= new File(uploadPath,
					fileItem.getName()); 
	 ...
	 * @throws Exception 
	 */
	public File doUpload(FileItem fileItem,InputStream inputStream,ClassLoader loader,Upload uploadConf) throws IOException, SecretKeyNotExists, InvalidKeyException, IllegalBlockSizeException, BadPaddingException{
		/**
		 * 字符串变量赋值
		 */
		...
		if(secretKeyStr.equals("")){
			...
			File sf = new File(secretKeyStr);
			if(!sf.exists())throw new SecretKeyNotExists("the secret key file is not exists!");
			SecretKey secretKey = DH.getKey(sf);
			this.doUpload(fileTmp,inputStream,secretKey);
		}
		return fileTmp;
	}
	public void doUpload(File fileTmp, InputStream inputStream, SecretKey secretKey) throws InvalidKeyException, IllegalBlockSizeException, BadPaddingException, IOException {
		FileOutputStream fos = new FileOutputStream(fileTmp);
		byte[] fileBuffer = new byte[1024];
		int line=-1;
		while ((line = inputStream.read(fileBuffer)) != -1){
			fos.write(DH.encryptByte(fileBuffer, secretKey), 0, line);
		}
		fos.flush();
		fos.close();
		inputStream.close();
		...
		try {
			if (loader.hasMethod(ClassLoader.createFieldSetMethod(field),
				...
				loader.set(field, file);
					...
					if(cLoader.hasMethod(ClassLoader.createFieldSetMethod(field),
							File.class)){
