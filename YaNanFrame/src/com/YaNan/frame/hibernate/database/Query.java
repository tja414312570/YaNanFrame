...
public class Query extends OperateImplement {
	...
	public Query addCondition(String field, Object condition) {
		if (condition == null)
			throw new RuntimeException("query condition is null at column " + field);
		field = field.indexOf(".") > -1 ? field : dataTables.getName() + "." + dataTables.getDBColumn(field).getName();
		if (condition.getClass().isArray()) {
			Object[] objects = (Object[]) condition;
			if (objects.length == 0)
				return this;
			StringBuffer sb = new StringBuffer(objects.length > 1 ? "(" : "");
			for (int i = 0; i < objects.length; i++) {
				sb.append(field).append(" = '").append(objects[i]).append("'");
				if (objects.length > 1) {
			...
			this.condition.add(sb.toString());
		} else {
	...
	public String create() {
		...
		if (this.joinObject != null) {
			...
			for (int i = 0; i < this.joinObject.getConditions().length; i++)
				sb.append(this.joinObject.getConditions()[i])
						.append(i < this.joinObject.getConditions().length - 1 ? " AND " : " ");
		}
		if (map.size() != 0) {
			sb.append(" WHERE ");
	...
	// HashMap<String,Object>(this.fieldMap.size());
